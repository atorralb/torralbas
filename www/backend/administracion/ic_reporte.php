...
function connect($host='localhost',$username='',$password='',$db=''){
	$this->conn = $mysqli = new mysqli($host,$username,$password) or die( mysql_error() );
	mysqli_select_db($db,$this->conn) or die( mysql_error() );
	return true;
}

function query($query){
	$this->results = mysqli_query($mysqli,$query,$this->conn);
...
sum(if(almacen1=$almacen AND fecha <$da$dm$dd, cantidad, 0)) - sum(if(almacen2=$almacen AND fecha <$da$dm$dd, cantidad, 0))  as inventarioinicial, 
...
, productos.cprov, productos.cprod, productos.descripcion
from eys, productos where eys.cprod = productos.cprod and eys.cprov = productos.cprov group by productos.cprov, productos.cprod;");

$pdf = new PDF('L','pt','A4');
$pdf->SetFont('Arial','',9);
$pdf->AliasNbPages();
$pdf->connect('localhost','root','','inventario');

$attr=array('titleFontSize'=>14,'titleText'=>'reporte de inventario completo del '."$da$dm$dd".' hasta');
$pdf->mysql_report("select cprov, cprod,  inventarioinicial, entradas, salidas, inventariofinal from inventario",  false, $attr);
?>