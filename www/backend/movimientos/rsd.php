...
<BODY>
<form action=rsd.php method=get>
<table>
...
if($action=="del"){mysqli_query($mysqli,"DELETE FROM rsd WHERE id=$id;");}
...
if($cprov != "" && $cprod != "" && $cantidad != "") { mysqli_query($mysqli,"INSERT INTO rsd (f, cprov, cprod, cantidad) VALUES ('$folio', '".$existeproducto['cprov']."', '".$existeproducto['cprod']."', '$cantidad' );");}
...
rsd.cantidad*productos.costodecompra AS spcc,  rsd.id, rsd.cprov, rsd.cprod, rsd.cantidad, productos.descripcion, productos.costodecompra
FROM 
rsd, productos 
WHERE 
rsd.cprov = productos.cprov 
AND 
rsd.cprod = productos.cprod 
AND 
rsd.f = $folio
...
while( $row=mysqli_fetch_array($result))
	{
		...
		echo "<tr><td>".$row['cantidad']."</td><td>".$row['cprov']."</td><td>".$row['cprod']."</td><td>$".$row['spcc']."</td><td>$ $sppv</td><td>".$row['descripcion']."</td><td>$".$row['costodecompra']."</td><td>$".$row['pv']."</td><td><a href=rsd.php?action=del&id=".$row['id'].">eliminar</a></td></tr>";
		echo "\n";
	}					
?>
</TABLE>
<a href='rsd.php?action=fin'>finalizar sesion</a>
<?if($action == "fin") { 
session_destroy(); 
mysqli_query($mysqli,"delete from rsd where f = $folio and cprov ='' and cprod =0;");
echo "<br><b>SESION FINALIZADA</b> <br><a href ='sdi.php'>empezar nuevo folio</a><p>";}?>
</BODY>
</HTML>