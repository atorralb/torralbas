...
<HTML>
<head>
<a href='rst.php?action=fin'>finalizar sesion</a>
<? if($action == "fin")	{session_destroy(); 
mysqli_query($mysqli,"delete from rst where f = $folio and cprov ='' and cprod =0;");
echo "<br><b>SESION FINALIZADA</b><br><a href ='sti.php'>empezar nuevo folio</a>";} ?>
</head>
<STYLE>
@import url(../gui.css);
</STYLE>
...
<form action=rst.php method=get>
...
if($action=="del")	{mysqli_query($mysqli,"DELETE FROM rst WHERE id=$id;");}
...
if($cprov != "" && $cprod != "" && $cantidad != "") { mysqli_query($mysqli,"INSERT INTO rst (f, cprov, cprod, cantidad) VALUES ('$folio', '".$existeproducto['cprov']."', '".$existeproducto['cprod']."', '$cantidad' );");}
...
rst.cantidad*productos.costodecompra AS spcc,  rst.id, rst.cprov, rst.cprod, rst.cantidad, productos.descripcion, productos.costodecompra
FROM 
rst, productos 
WHERE 
rst.cprov = productos.cprov 
AND 
rst.cprod = productos.cprod 
AND 
rst.f= $folio
ORDER BY id DESC LIMIT 10;");	
	...
	echo "<tr><td>".$row['cantidad']."</td><td>".$row['cprov']."</td><td>".$row['cprod']."</td><td>$".$row['spcc']."</td><td>$ $sppv</td><td>".$row['descripcion']."</td><td>$".$row['costodecompra']."</td><td>$".$row['pv']."</td><td><a href=rst.php?action=del&id=".$row['id'].">eliminar</a></td></tr>";
	echo "\n";
	}					
?>
</TABLE>
